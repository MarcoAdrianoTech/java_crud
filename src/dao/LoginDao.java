...
public class LoginDao {
    ...
    public void cadastrar(loginModel obj) {
        try {
            String sql = "insert into tb_usuario(nomecard,fun_designer,email,quantidade,qtd_cards,total,tipo,senha,data_atual)value(?,?,?,?,?,?,?,?,?)";
            ...
            stmt.setString(9,obj.getData_atual());
    ...
    public List<loginModel> listarCadastro() {
        ...
        try {
          ...
          //  String sql = "select * from tb_usuario";
            String sql = " select usa.id, usa.nomecard, usa.tipo, usa.fun_designer, usa.email, usa.quantidade, usa.qtd_cards, usa.total, usa.senha, date_format(usa.data_atual,'%d%/%m%/%y%')as data_formatada from tb_usuario usa";
            ...
            while (rs.next()) {
                ...
                obj.setData_atual(rs.getString("data_formata"));
     ...
     public void alterar(loginModel obj) {
        try {
            String sql = "update tb_usuario set nomecard=?,fun_designer=?,email=?,quantidade=?,qtd_cards=?,total=?,tipo=?,senha=?,data_atual=? where id=?";
      ...
      public List<loginModel> buscaporNome(String nomecard) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                obj.setTotal(rs.getDouble("total"));
                obj.setSenha(rs.getString("senha"));
                obj.setData_atual(rs.getString("data_atual"));
